...
**方法一：** 最直接的下载方式是通过git clone或者链接来下载，但是因为llm每部分都按GB计算，避免出现 `OOM Error(Out of memory)`，我们可以使用 Python 写一个简单的 download.py。
...
**方法二：** 通过Hugging Face Command Line Interface (CLI) 工具下载模型，具体可以参考官网的介绍 [cli (huggingface.co)](https://huggingface.co/docs/huggingface_hub/guides/cli)  

在 macOS 和 Linux 平台下载 CLI：
...
下载完成后我们就可以使用该工具完成特定模型的下载，这里以下载 Qwen/Qwen1.5-0.5B 为例：

```bash
hf download Qwen/Qwen1.5-0.5B
```

下载完成后可以看到文件保存的路径，如下图：

![alt text](../images/C3-1-14.png)

我们可以从上述文件夹下找到该模型的所有文件，可以拷贝到其他文件夹中备用。
...
```python
ollama create mymodel4 -f ./Modelfile
```
