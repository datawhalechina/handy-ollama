...
4. 终端内运行模型（实测在notebook中运行不出来，最好在终端运行）
    ...
    ollama run mymodel
    ```
![alt text](../images/C3/image-1.png)
...
> safetensors文件下载入口：
Qwen0.5b（可自行尝试）：https://huggingface.co/Qwen/Qwen1.5-0.5B/resolve/main/model.safetensors
llama3-8b-bnb-4bit（演示使用）：https://huggingface.co/unsloth/llama-3-8b-bnb-4bit/resolve/main/model.safetensors

![alt text](../images/C3/image-13.png)

由于这部分内容社区还在不断优化中，因此，这里提供的示例代码和流程仅供参考，并不保证能成功运行。

1. 根目录创建文件夹名为model，将下载好的safetensors文件拖入(这里的safetensors文件只能以model.safetensors命名)
...
![alt text](../images/C3/image-2.png)
![alt text](../images/C3/image-3.png)
![alt text](../images/C3/image-4.png)
...
![alt text](../images/C3/image-5.png)
...
```
![alt text](../images/C3/image-6.png)
...
```
![alt text](../images/C3/image-7.png)
到此，我们的模型就量化并创建完成了
...
```
![alt text](../images/C3/image-8.png)
...
2. 创建模型
    ...
    ollama create mymodel -f ./Modelfile
    ```
    ![alt text](../images/C3/image-9.png)
    创建模型的时间可能稍微会久一点，和pull一个模型的时间差不多，请耐心等待
    ![alt text](../images/C3/image-10.png)
    再次运行`ollama list`查看已有的模型，可以看到mymodel已经正确创建了
    ![alt text](../images/C3/image-11.png)
3. 运行模型
    ```python
    ollama run mymodel
    ```
    ![alt text](../images/C3/image-12.png)
